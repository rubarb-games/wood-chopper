...
public class MeshSlicer {
    ...
    public void Run(Vector3[] verts, Vector3 normal, Vector3 q, ref List<Vector3> front, ref List<Vector3> back) {
        Vector3 previous     = verts[^1];
        ...
        for (int i = 0; i < verts.Length; ++i) {
            ...
            if (InFrontOfPlane(distCurrent)) {
                if (AtBackOfPlane(distPrevious)) {
                    Vector3 vi = current.Lerp(previous, distCurrent / (distCurrent - distPrevious));
                    front.Add(vi);
                    back.Add(vi);
                }
                ...
                if (InFrontOfPlane(distPrevious)) {
                    Vector3 vi = previous.Lerp(current, distPrevious / (distPrevious - distCurrent));
                    front.Add(vi);
                    back.Add(vi);
                }
...
//                }
    ...
    private static float DistToPlane(Vector3 p, Vector3 q, Vector3 n) {
        // return Vector3.Dot(p - q, n) / n.magnitude;
        return (p - q).Dot(n) / n.Length();
    }

    private static bool InFrontOfPlane(float dist) {
        return dist > float.Epsilon;
    }

    private static bool AtBackOfPlane(float dist) {
        return dist < float.Epsilon;
    }

    private static bool OnPlane(float dist) {
